...
public class MerchPrizeTestWLAEngineManagerTest
{
	...
	{
		logger = Logger.getLogger(MerchPrizeTestWLAEngineManagerTest.class);
		gameParams = XMLUtil.parseResource("paymodel/MerchPrizeTestWLAIW/GameParams.xml").getDocumentElement();
		previousOutcome = XMLUtil.parseResource("paymodel/MerchPrizeTestWLAIW/InitOutcome.xml").getDocumentElement();
		...
		throws Exception
	{
		...
		// Scenario
		playerInput = new PlayerInputBuilder().appendActionInput("play").build();
		TestODEProxy.setJsonResponse("{'prizeValue':101767900,'prizeDivision':1}");
		outcome = OutcomeUtil.getSignedData(outcome, "Outcome");
		outcome = manager.play(InputUtil.buildInput(gameParams, XMLUtil.getElement(outcome), playerInput));
		logger.debug(outcome);
		...
		// MerchandisePrize
		playerInput = new PlayerInputBuilder().appendNonCashPrizeInput("01-01-01", "Volvo XC60 Hybrid", 1, 50).build();
			...
			throws Exception
	{
		...
		playerInput = new PlayerInputBuilder().appendNonCashPrizeInput("01-01-01", "Volvo XC60 Hybrid", 1, 100).build();
		outcome = OutcomeUtil.getSignedData(outcome, "Outcome");
		outcome = manager.play(InputUtil.buildInput(gameParams, XMLUtil.getElement(outcome), playerInput));
		logger.debug(outcome);
